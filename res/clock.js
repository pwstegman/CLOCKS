function Clock(x, y, radius, options) {
	...
	var clockShape = drawClock(x, y, radius, options);
...
function drawClock(x, y, radius, options) {
	...
	var minuteHand = two.makeLine(0, 0, options.minuteLength * Math.cos(dtr(minuteAngle)), options.minuteLength * Math.sin(dtr(minuteAngle)))
	minuteHand.stroke = options.minuteColor
	...
	var hourHand = two.makeLine(0, 0, options.hourLength * Math.cos(dtr(hourAngle)), options.hourLength * Math.sin(dtr(hourAngle)))
	hourHand.stroke = options.hourColor
